...
Write-Output "$PC Installing AudioDeviceCmdlets from (mefranklin6 fork)"

$AudioDeviceCmdletsModuleName = 'AudioDeviceCmdlets'
try {

    $Version = ConvertTo-NormalizedVersion -Version $Version
    $AudioDeviceCmdletsZipUrl = "https://github.com/mefranklin6/$AudioDeviceCmdletsModuleName/releases/download/v$Version/$AudioDeviceCmdletsModuleName-$Version.zip"
    ...
    if ($null -eq $installed) {
        throw "$PC AudioDeviceCmdlets (mefranklin6 fork) not installed"
    }
    else {
        Write-Output "$PC AudioDeviceCmdlets (mefranklin6 fork) installed ($($installed.Version))"
    }

} # end try
catch {
    Write-Output "$PC InstallAudioDeviceCmdlets (mefranklin6 fork) failed: $_"
    Exit 1
}
